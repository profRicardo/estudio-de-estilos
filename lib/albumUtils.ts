...
function loadImage(src: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        ...
        img.onerror = (err) => reject(new Error(`Falha ao carregar imagem: ${src.substring(0, 50)}...`));
        img.src = src;
    });
}
...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillText('As Suas Escolhas de Estilo', canvasWidth / 2, 150);

    ctx.font = `60px 'Poppins', sans-serif`;
    ctx.fillStyle = '#555';
    ctx.fillText('do Estúdio de Estilos', canvasWidth / 2, 220);
